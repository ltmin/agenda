...
var Agenda = /** @class */ (function (_super) {
    ...
    function Agenda(config, cb) {
        ...
        if (config.mongo) {
            _this.mongo(config.mongo, config.db ? config.db.collection : undefined, cb); // @ts-expect-error // the documentation shows it should be correct: http://mongodb.github.io/node-mongodb-native/3.6/api/Db.html
            if (config.mongo.s && config.mongo.topology && config.mongo.topology.s) {
                _this._mongoUseUnifiedTopology = Boolean(
                // @ts-expect-error
